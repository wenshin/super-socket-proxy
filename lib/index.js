const debug = require('debug')('super-socket-proxy');
...
const SocketProxy = {
  ...
  createServer(options) {
    ...
    options && options.port && server.listen(options.port, options.host);
  ...
  createClientServer(options) {
    ...
    console.log(`[SocketProxy]forward ${host || 'localhost'}:${port} to ${targetName || 'target server'} ${targetHost}:${targetPort}`);
    const server = net.createServer((socket) => {
      SocketProxy.connect({
        host: targetHost,
        port: targetPort,
  ...
  connect(options = {}, onConnect) {
    ...
    assert.ok(parseInt(options.proxyPort, 10), 'proxy target server port is necessary');
